...
def mostrar_login():
    ...
    st.info("¿Eres admin? Puedes registrar nuevos usuarios abajo.")
    if st.button("Registrar usuario (solo admin)"):
...
def mostrar_login():
    """
    Muestra el formulario de login. Si credenciales válidas, inicia sesión y registra bitácora.
    """
    st.title("🔐 Inicio de sesión")
    usuario = st.text_input("Usuario")
    password = st.text_input("Contraseña", type="password")
    ...
    if st.button("Ingresar"):
        ...
        if user:
            ...
            hora_ingreso = datetime.now()
            st.session_state["hora_ingreso"] = hora_ingreso
            # --- Registra inicio de sesión en bitácora ---
            registrar_bitacora(
                usuario=user["usuario"],
                tabla="usuarios",
                tipo_accion="login",
...
def mostrar_registro():
    """
    Formulario para registrar un usuario (solo visible para admin).
    ...
    if st.button("Registrar"):
        if not nombre or not usuario or not password:
            ...
            cur.close()
            conn.close()
            # --- Registra creación de usuario en bitácora ---
            registrar_bitacora(
                usuario=st.session_state["usuario"]["usuario"],
                tabla="usuarios",
                tipo_accion="crear",
